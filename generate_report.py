...
def _card_cockpit_html(
    ...
    *,
    ...
    wenn CSS noch nicht geladen ist. (Iterativ verkleinert: 185 -> 160
    -> 150 -> 135 -> 120 px nach iPhone-Live-Verify; Conviction-Zahl
    proportional 50 -> 42 -> 40 -> 36 -> 32 px nachgezogen.)
    """
    ticker = s.get("ticker", "?")
    price = _safe_float(s.get("price", 0))
    chg = _safe_float(s.get("change", 0))
...
function _btRender(){{
  ...
  let metaHtml = '<b>' + nTot + ' Datenpunkte</b> — davon <b>' + nBoot
    ...
    + '<span style="display:block;margin-top:3px;font-size:.72rem;color:#94a3b8">'
    + '90-Tage-Rollfenster — die Zahl kann sinken, ohne dass Daten verloren gehen; '
    + 'gereifte Records bleiben dauerhaft prune-immun gesichert (seit 04.08.).</span>'
...
function _quotePatchScope(scope, ticker, price, changePct, changeAbs) {{
  ...
  if (typeof price === 'number' && isFinite(price)) {{
    scope.querySelectorAll('.pos-pnl-live').forEach(el => {{
      ...
      // Frischer Live-Quote → etwaigen Build-Zeit-Stale-Marker entfernen:
      // der gezeigte Preis ist jetzt live, „Kurs vom …" gilt nicht mehr.
      el.classList.remove('pos-pnl-stale');
    }});
    scope.querySelectorAll('.pos-stale-note').forEach(el => el.remove());
  }}
...
function _goodFriday(year) {{
  // Meeus/Jones/Butcher-Osterformel, JS-Spiegel von config._easter_sunday.
  // Bit-exakt gegen echten NYSE-Kalender für 2020-2030 verifiziert (Test A).
...
(function() {{
  function renderAgentSignals(data) {{
    ...
    document.querySelectorAll('.card[data-ticker]').forEach(card => {{
      ...
      // KI-Pillar-ZAHL live nachziehen (Fix 14.07.). Der frische ki-Score liegt
      // hier vor (signals[ticker].score) und wird bereits für den Dot genutzt —
      // die server-gerenderte KI-Zahl kann aber "—" sein (Neu-Einsteiger nach
      // Top-10-Rotation: Daily-Run rendert VOR dem Tick, der die neue Top-10
      // scort) oder stale (Wert eines früheren Ticks). Zahl + Farbe + Balken
      // konsistent nachziehen — Farb-/Schwellen-Logik identisch zu server-
      // _tri_score_color (≥60 grün · ≥30 orange · <30 rot). Konfidenz-Wasser-
      // zeichen (sb-conf-*-Klasse + title/aria, Neutralisierung #425/#426)
      // bleibt UNBERÜHRT: nur textContent + style.color/width, KEIN classList-
      // Touch. Kein Score → "—" bleibt (kein Overwrite). Element fehlt → no-op.
      if (sig && sig.score != null) {{
        const _kiCol = score >= 60 ? '#22c55e' : score >= 30 ? '#f97316' : '#ef4444';
        const _kiPct = Math.max(0, Math.min(100, score));
        const _kiTxt = String(Math.round(score));
        // Aktiver Cockpit-Pfad (CARD_COCKPIT_ENABLED). Value hat KEIN Delta-Child
        // (Delta nur Setup) → textContent-Ersatz sicher.
        const _kv = card.querySelector('.cockpit-pillar[data-sb="ki"] .cockpit-pillar-value');
        ...
        const _v1f = card.querySelector('.sb-row[data-sb="ki"] .sb-fill');
        if (_v1f) {{ _v1f.style.width = _kiPct + '%'; _v1f.style.background = _kiCol; }}
      }}
...
(function(){{
  ...
  // Stale-Preis-Marker (Anzeige-Folge zu #483): der persistierte Daily-Run-
  // current_price einer Position wird bei einem Fetch-Fehler PRESERVED (sein
  // price_asof bleibt alt statt mit dem aktuellen Run mitzuwandern). Hier NUR
  // Anzeige — erkennen, ob der GEZEIGTE Preis dieser preservte Wert ist, und
  // dann gedimmt + „Kurs vom <Datum>" markieren. Liest price_asof, schreibt
  // NIE. Vergleich in UTC (beide Stempel sind …Z), Anzeige in Berlin.
  const _POS_PRICE_STALE_TOL_MS = 2 * 3600 * 1000;  // 2h: >> Same-Run-Jitter (~Sek), << Daily-Run-Abstand (~9h)
  function _positionStaleAsof(ticker, shownPrice) {{
    try {{
      const ph2   = (window._POSITIONS_DATA || {{}})[ticker];
      const runTs = (window._APP_DATA || {{}}).last_daily_run_ts;
      if (!ph2 || !ph2.price_asof || !runTs) return null;          // null-tolerant (Alt-State)
      const asofMs = Date.parse(ph2.price_asof), runMs = Date.parse(runTs);
      if (!isFinite(asofMs) || !isFinite(runMs)) return null;
      if (runMs - asofMs <= _POS_PRICE_STALE_TOL_MS) return null;  // frisch (gleicher Run) → kein Marker
      // Live-Quote-Guard: nur markieren, wenn der GEZEIGTE Preis der preservte
      // Wert ist (kein frischer Live-Overlay). shownPrice null → Marker erlaubt.
      const stored = (typeof ph2.current_price === 'number') ? ph2.current_price : null;
      if (stored != null && shownPrice != null && Math.abs(+shownPrice - stored) > 0.005) return null;
      return ph2.price_asof;
    }} catch (e) {{ return null; }}
  }}
  function _fmtStaleDate(iso) {{
    try {{
  ...
  function buildPositionPanel(ticker, currentPrice) {{
    ...
    if (pos) {{
      ...
      // Stale-Preis-Marker: nur wenn der gezeigte cp der preservte Daily-Run-
      // Wert ist (siehe _positionStaleAsof). Frisch/null → beide Strings leer
      // → Render byte-identisch zu vorher (kein optischer Unterschied).
      const _staleAsof   = _positionStaleAsof(ticker, cp);
      const _pnlStaleCls = _staleAsof ? ' pos-pnl-stale' : '';
      const _staleNote   = _staleAsof
        ? `<div class="pos-stale-note">⏱ Kurs vom ${{_fmtStaleDate(_staleAsof)}} — Fetch ausst\xe4ndig</div>`
        : '';
      return `<div class="position-panel position-panel-active">
        ...
        <div class="pos-grid">
          ...
          <div><span class="pos-lbl">P&amp;L</span><span class="pos-val pos-pnl-live${{_pnlStaleCls}}" data-ep="${{ep}}" style="color:${{pnlCol}};font-weight:700">${{pnlStr}}</span></div>
        </div>
        ${{_staleNote}}
        ${{errHtml}}
        <div class="pos-actions">
