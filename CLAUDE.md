...
| S7 | warn | Daily | ``agent_signals`` ∩ Top-10 ≥ ``HEALTH_CHECK_S7_MIN_AGENT_OVERLAP`` (5). **Race-Gate (23.06.2026):** SUPPRESS bei niedriger Überlappung, WENN seit dem vorigen Daily-Run schon ein ki_agent-Tick lief (``agent_signals.updated`` > ``app_data.last_daily_run_ts``) — das ist die erwartete Top-10-Rotation, kein Drift. FEUERN nur wenn kein Tick seit vorigem Run (Auto-Trigger-Kette gebrochen, 14.05.-Bug-Klasse) ODER ein Timestamp fehlt/unparsebar (konservativ). Mehrtages-Ausfall zusätzlich von S8 (26h) gefangen. ``last_daily_run_ts`` schreibt der Daily-Run in ``app_data.json``; ki_agent preserviert es via ``**existing``. |
