#!/usr/bin/env python3
"""Isolierter Rückweg für das ``ssr_restriction``-Sammelfeld (Rule-201, 28.07.2026).

Poppt EXAKT den einen benannten Top-Level-Key ``ssr_restriction`` aus jedem Record in
``backtest_history.json``. **Alle anderen Felder bleiben byte-identisch** — das
Feld wird ausschließlich vom Sammler in
``backtest_history._build_backtest_extension`` geschrieben, ist also eindeutig.

WARUM KEIN MANIFEST (analog ``purge_material_8k_events.py``): der Rückweg-Anker
ist ein **benannter JSON-Key** — ``rec.pop("ssr_restriction", None)`` trifft ausschließlich
dieses Feld, keine Recompute-Kollision, kein Manifest nötig. Alt-Records ohne
den Key (vor dem Feature) bleiben unberührt (pop no-op).

Läufe:
  ``python scripts/purge_ssr_restriction.py``          → DRY-RUN (Preview).
  ``python scripts/purge_ssr_restriction.py --live``   → schreibt die Datei.
...
_KEY = "ssr_restriction"

log = logging.getLogger("purge_ssr")
...
def purge_records(history: list) -> tuple[list, int]:
    """Pure: poppt ``ssr_restriction`` aus jedem Record. Gibt ``(Liste-Referenz, n_popped)``
    zurück. Mutiert die Dicts in-place (der Aufrufer hat frisch geladen)."""
    n = 0
    for rec in history:
