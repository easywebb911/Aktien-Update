#!/usr/bin/env python3
"""Isolierter Rückweg für ``source_pools`` (25.07.2026).

Poppt EXAKT den benannten Top-Level-Key ``source_pools`` aus jedem Record in
``backtest_history.json``. **Alle anderen Felder bleiben byte-identisch** —
``source_pools`` wird ausschließlich vom Record-Builder in
...
``purge_material_8k_events.py``). Kein zweites Artefakt (anders als
``purge_attention_wiki.py``, das zusätzlich die Ticker-Map löscht).

Alt-Records ohne den Key (vor dem Feature) bleiben unberührt (pop no-op).

Läufe:
  ``python scripts/purge_source_pool.py``          → DRY-RUN (Preview).
  ``python scripts/purge_source_pool.py --live``   → schreibt.
  ``--path X`` überschreibt die backtest-Datei (Tests).
...
log = logging.getLogger("purge_source_pool")
...
def purge_records(history: list) -> tuple[list, int]:
    """Pure: poppt ``source_pools`` aus jedem Record. Mutiert die Dicts
...
def main(argv=None) -> int:
    ...
    if not args.live:
        log.info("DRY-RUN: nichts geschrieben. Für echt: --live.")
        return 0
